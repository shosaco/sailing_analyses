...
make_hist <- function(fill_var, dat = all_dat){
  ...
  highchart() %>% hc_chart(type = "column") %>%
    ...
    hc_title(text = glue("Number of sessions per {fill_var}")) %>% 
    hc_yAxis(text = NULL) %>% 
...
levels <- metadata %>% group_by(Boot) %>%
  summarize(meanspeed = mean(mean_speed, na.rm=T), .groups = "drop") %>% 
...
pirateplot(formula = mean_speed ~ Boat,
...
Data collection was carried out using the Apps [Komoot](https://www.komoot.com) and [Waterspeed](https://www.waterspeedapp.com/), this analysis was conducted using [R 4.0](https://www.r-project.org/) and the following useful helpers:
...
All Code for calculations and visualisations can be downloaded in my Github Repository: [https://github.com/shosaco/sailing_analyses](https://github.com/shosaco/sailing_analyses), this page is available at [https://shosaco.github.io/sailing_analyses](https://shosaco.github.io/sailing_analyses).
