...
subtitle: "What can we learn from 220 dinghy miles?<br/>(Alternative title: That's how a sailing data scientist spends his time in autumn!)"
author: Sandro Raabe
...
With `r  dat_boote$n` sessions *`r dat_boote$Boot`* was my favourite boat. This has two reasons: On the one hand it's a very beginner-friendly boat (I only did my certification in autumn 2019 and have never touched a sail boat before) - the other reason you can find in the [analysis of sailing partners](#frequency-distribution-2) later in this article.
...
An interesting question arises: which boat has - on average - which speed (measured in knots)?

```{r pirateplot, fig.height=5.5, fig.width=12}
levels <- all_dat %>% group_by(Boot) %>%
  summarize(meanspeed = mean(speed, na.rm=T), .groups = "drop") %>% 
...
pirateplot(formula = speed ~ Boat, #point.col = "#6B6B6B",
           data = dat, inf.f.o = 0.1, point.o = .005) 
```

Interesting to note: *Kielzugvogel* had the highest average (as seen on the bellies of the distribution), but not the highest median speed (thick black line). This is due to outliers in maximum speed, that we encountered using the Zugvogel (median doesn't care about outliers, mean does).

**Observation:** Interestingly, the extremes in terms of weight are also the fastest: The lightweight high performance dinghy *470er* and the massive keel boats *Kielzugvogel* and *C55*, whereas the former beats all other boats in terms of top speed by far. The *J70* is the Bundesliga competition boat and should normally be faster, but on that particular day there wasn't much wind apparently :-)
...
* Is there any correlation between wind speed and boat types? Do I prefer certain boats for certain wind conditions?
